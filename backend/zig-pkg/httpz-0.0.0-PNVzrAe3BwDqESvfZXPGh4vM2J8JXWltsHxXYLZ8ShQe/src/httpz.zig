...
pub fn Server(comptime H: type) type {
    ...
    return struct {
        ...
        pub fn listen(self: *Self) !void {
            ...
            const listener = blk: {
                // Windows 的 ws2_32.SOCK 里没有 CLOEXEC 常量；
                // 这里改成按平台拼接标志位，避免在 Zig 0.16 + Windows 下直接编译失败。
                var sock_flags: u32 = posix.SOCK.STREAM;
                if (builtin.os.tag != .windows) {
                    sock_flags |= posix.SOCK.CLOEXEC;
                }
                // 同理，Windows 的 SOCK 常量集合里也没有 NONBLOCK。
                // httpz 自己的 posix.socket 已经会在 Windows 分支做专门处理，
                // 这里仅在非 Windows 平台继续透传该标志位。
                if (blockingMode() == false and builtin.os.tag != .windows) {
