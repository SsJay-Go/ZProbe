...
// Router 是 httpz 的路由核心。
// 它按 HTTP method 拆分路由树，并把 path、handler、middleware 组装成可调度动作。
pub fn Router(comptime Handler: type, comptime Action: type) type {
    ...
    return struct {
        ...
        // 这里默认假设 allocator 是 Arena，便于路由表一次性构建后长期复用。
