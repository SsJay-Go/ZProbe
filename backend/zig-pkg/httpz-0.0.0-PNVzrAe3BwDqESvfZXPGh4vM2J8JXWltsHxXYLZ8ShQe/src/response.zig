...
// Response 是业务层写回 HTTP 响应的主接口。
// 它负责聚合状态码、头、正文和 chunk / event stream 等不同输出模式。
pub const Response = struct {
    // httpz 对底层连接的包装；需要底层 stream 时仍可继续下探。
    conn: *HTTPConn,
    ...
    // Response 由连接池和请求处理流程统一初始化，不建议业务代码直接手动构造。
    ...
    // json 是业务层最常用的便捷输出函数：序列化并自动标记 JSON content-type。
    ...
    // startEventStream 用于 SSE 等持续推送场景。
    // 它会先写响应头，再把连接所有权交给后台线程持续输出。
