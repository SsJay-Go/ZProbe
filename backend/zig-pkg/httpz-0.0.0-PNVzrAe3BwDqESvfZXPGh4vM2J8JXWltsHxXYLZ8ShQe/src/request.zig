...
// Request 是 httpz 暴露给业务 handler 的“请求视图”。
// 它把底层连接、URL、头、参数、查询串和请求体组织成一份便于业务读取的结构。
pub const Request = struct {
    // 请求 URL 的解析结果。
    url: Url,

    // httpz 对底层连接的包装；必要时仍可深入访问其 stream。
    conn: *HTTPConn,

    // 客户端地址。
    address: Address,

    // 路由参数，例如 /device/:id 中抽出来的 id。
    // 业务层优先通过 req.param(name) 访问，而不是直接碰底层表结构。
    params: *Params,
    ...
    // init 会把连接状态快照整理成一份供业务读取的 Request 结构。
    ...
    // 依据协议版本和 Connection 头判断当前连接是否还能复用。
